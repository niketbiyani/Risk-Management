    ...
    try {
      if (iframe.contentDocument && !iframe.dataset.rmExtListenerAttached) {
        ...
        const handlePaneClick = (e) => {
          ...
          // 1. Try scanning clicked chart box directly
          const clickedSym = scanChartFromClick(e);
          if (clickedSym) {
            chrome.runtime.sendMessage({ type: 'ACTIVE_SYMBOL_DETECTED', symbol: clickedSym }).catch(() => {});
            return;
          }

          // 2. Fallback to active widget scanner
          setTimeout(() => {
            const sym = scanActiveSymbol();
            if (sym) {
              chrome.runtime.sendMessage({ type: 'ACTIVE_SYMBOL_DETECTED', symbol: sym }).catch(() => {});
