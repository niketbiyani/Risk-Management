"""
Dhan 20-Level Depth WebSocket Engine.

Connects to Dhan's depth feed, parses binary order book data, and provides
real-time analytics: imbalance, wall detection, absorption/pull/stacking alerts,
cumulative delta, tick reconstruction, and volume profile.

Standalone — no dependency on Risk-Management internals.
"""

import asyncio
import csv
import io
import json
import logging
import struct
import threading
import time
from collections import deque
from dataclasses import dataclass
from datetime import datetime

import requests
import websockets

from config import Config

logger = logging.getLogger(__name__)

# WebSocket endpoint
DEPTH_WS_URL = "wss://depth-api-feed.dhan.co/twentydepth"

# Feed codes in binary packets
FEED_CODE_BID = 41
FEED_CODE_ASK = 51

# Analytics constants
WALL_THRESHOLD_MULTIPLIER = 3.0
STACKING_THRESHOLD_MULTIPLIER = 2.5
DELTA_HISTORY_SIZE = 300
TICK_HISTORY_SIZE = 600
ALERT_HISTORY_SIZE = 50

# Scrip master URL
SCRIP_MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"


# ── Instrument Lookup ─────────────────────────────────────────────

@dataclass
class Instrument:
    security_id: str
    trading_symbol: str
    symbol_name: str
    exchange: str
    instrument_type: str
    lot_size: int
    expiry_date: str
    exchange_segment: str


def find_nifty_futures() -> Instrument | None:
    """Download scrip master and find nearest-month Nifty futures."""
    logger.info("Downloading Dhan scrip master for instrument lookup...")
    try:
        resp = requests.get(SCRIP_MASTER_URL, timeout=60)
        resp.raise_for_status()
    except Exception as e:
        logger.error("Failed to download scrip master: %s", e)
        return None

    reader = csv.DictReader(io.StringIO(resp.text))
    today = datetime.now()
    best = None
    best_days = 999

    for row in reader:
        exchange = row.get("SEM_EXM_EXCH_ID", "")
        inst_type = row.get("SEM_INSTRUMENT_NAME", "")
        symbol = row.get("SM_SYMBOL_NAME", "")

        if exchange != "NSE" or inst_type != "FUTIDX" or symbol != "NIFTY":
            continue

        expiry = row.get("SEM_EXPIRY_DATE", "")
        if not expiry:
            continue

        try:
            exp_date = datetime.strptime(expiry, "%Y-%m-%d")
            days = (exp_date - today).days
            if 0 <= days < best_days:
                segment = row.get("SEM_SEGMENT", "")
                exchange_segment = f"{exchange}_FNO" if segment == "D" else exchange
                try:
                    lot_size = int(float(row.get("SEM_LOT_UNITS", "75") or "75"))
                except (ValueError, TypeError):
                    lot_size = 75

                best = Instrument(
                    security_id=row.get("SEM_SMST_SECURITY_ID", ""),
                    trading_symbol=row.get("SEM_TRADING_SYMBOL", ""),
                    symbol_name=symbol,
                    exchange=exchange,
                    instrument_type=inst_type,
                    lot_size=lot_size,
                    expiry_date=expiry,
                    exchange_segment=exchange_segment,
                )
                best_days = days
        except ValueError:
            continue

    if best:
        logger.info("Found Nifty Futures: %s (ID: %s, Lot: %d, Expiry: %s)",
                     best.trading_symbol, best.security_id,
                     best.lot_size, best.expiry_date)
    else:
        logger.warning("Could not find Nifty futures in scrip master")

    return best


# ── Depth Analyzer ────────────────────────────────────────────────

class DepthEngine:
    """20-level depth WebSocket client with order book analytics."""

    def __init__(self, socketio):
        self._socketio = socketio

        # Connection state
        self._running = False
        self._connected = False
        self._ws = None
        self._thread = None

        # Instrument
        self._security_id = None
        self._trading_symbol = ""
        self._lot_size = 75
        self._expiry = ""

        # Order book (20 levels each side)
        self._bids = [{"price": 0.0, "qty": 0, "orders": 0} for _ in range(20)]
        self._asks = [{"price": 0.0, "qty": 0, "orders": 0} for _ in range(20)]
        self._total_bid_qty = 0
        self._total_ask_qty = 0
        self._last_update = 0.0

        # Previous state (for delta / absorption tracking)
        self._prev_bids = None
        self._prev_asks = None

        # Time-series analytics
        self._delta_history = deque(maxlen=DELTA_HISTORY_SIZE)
        self._cumulative_delta = 0.0
        self._alerts = deque(maxlen=ALERT_HISTORY_SIZE)

        # Tick reconstruction (inferred trades from book changes)
        self._tick_history = deque(maxlen=TICK_HISTORY_SIZE)
        self._volume_profile = {}  # price -> {"buy": qty, "sell": qty}

        # Throttle SocketIO emissions to max ~4/sec
        self._last_emit = 0.0
        self._emit_interval = 0.25

    # ── Lifecycle ────────────────────────────────────────────────

    def start(self):
        """Start the depth engine in a background thread."""
        if self._running:
            return

        if not self._security_id:
            inst = find_nifty_futures()
            if not inst:
                logger.error("Cannot start — no instrument found")
                return
            self._security_id = inst.security_id
            self._trading_symbol = inst.trading_symbol
            self._lot_size = inst.lot_size
            self._expiry = inst.expiry_date

        self._running = True
        self._thread = threading.Thread(
            target=self._run_loop, daemon=True, name="depth-engine"
        )
        self._thread.start()
        logger.info("Depth engine thread started")

    def stop(self):
        """Stop the depth engine."""
        self._running = False
        self._connected = False
        logger.info("Depth engine stopping...")

    def reconnect(self):
        """Force reconnect (e.g. after token refresh)."""
        self._connected = False
        logger.info("Reconnect requested")

    # ── Public Getters ───────────────────────────────────────────

    def get_config(self):
        return {
            "security_id": self._security_id,
            "trading_symbol": self._trading_symbol,
            "lot_size": self._lot_size,
            "expiry": self._expiry,
            "connected": self._connected,
            "running": self._running,
        }

    def get_snapshot(self):
        """Full order book snapshot with computed analytics."""
        bids = [b.copy() for b in self._bids if b["price"] > 0]
        asks = [a.copy() for a in self._asks if a["price"] > 0]
        walls = self._detect_walls(bids, asks)

        ltp = 0.0
        spread = 0.0
        if bids and asks:
            ltp = round((bids[0]["price"] + asks[0]["price"]) / 2, 2)
            spread = round(asks[0]["price"] - bids[0]["price"], 2)

        return {
            "symbol": self._trading_symbol,
            "security_id": self._security_id,
            "lot_size": self._lot_size,
            "expiry": self._expiry,
            "connected": self._connected,
            "bids": bids,
            "asks": asks,
            "ltp": ltp,
            "spread": spread,
            "total_bid_qty": self._total_bid_qty,
            "total_ask_qty": self._total_ask_qty,
            "timestamp": self._last_update,
            "imbalance_5": self._calc_imbalance(bids, asks, 5),
            "imbalance_10": self._calc_imbalance(bids, asks, 10),
            "imbalance_20": self._calc_imbalance(bids, asks, 20),
            "walls": walls,
            "cumulative_delta": self._cumulative_delta,
            "delta_history": list(self._delta_history)[-60:],
            "alerts": list(self._alerts)[-20:],
            "tick_history": list(self._tick_history)[-120:],
            "volume_profile": self._get_volume_profile(),
        }

    # ── WebSocket Client ─────────────────────────────────────────

    def _run_loop(self):
        asyncio.run(self._ws_client())

    async def _ws_client(self):
        logger.info("WebSocket client started")

        while self._running:
            try:
                token = Config.DHAN_ACCESS_TOKEN
                client_id = Config.DHAN_CLIENT_ID
                if not token or not client_id:
                    logger.warning("Missing credentials, waiting...")
                    await asyncio.sleep(10)
                    continue

                url = (f"{DEPTH_WS_URL}?token={token}"
                       f"&clientId={client_id}&authType=2")
                logger.info("Connecting to 20-level depth feed...")

                async with websockets.connect(
                    url, ping_interval=30, ping_timeout=10
                ) as ws:
                    self._ws = ws
                    self._connected = True
                    logger.info("Connected!")

                    await self._subscribe(ws)

                    while self._running and self._connected:
                        try:
                            msg = await asyncio.wait_for(ws.recv(), timeout=5.0)
                            if isinstance(msg, bytes):
                                self._process_binary(msg)
                            else:
                                logger.debug("Text: %s", msg[:200])
                        except asyncio.TimeoutError:
                            continue
                        except websockets.ConnectionClosed:
                            logger.warning("Connection closed by server")
                            break

            except Exception as e:
                logger.error("Connection error: %s", e)

            self._connected = False
            self._ws = None

            if self._running:
                logger.info("Reconnecting in 5s...")
                await asyncio.sleep(5)

    async def _subscribe(self, ws):
        sub_msg = {
            "RequestCode": 23,
            "InstrumentCount": 1,
            "InstrumentList": [{
                "ExchangeSegment": "NSE_FNO",
                "SecurityId": str(self._security_id),
            }],
        }
        await ws.send(json.dumps(sub_msg))
        logger.info("Subscribed to %s (ID: %s)",
                     self._trading_symbol, self._security_id)

    # ── Binary Parsing ───────────────────────────────────────────

    def _process_binary(self, data):
        offset = 0
        while offset < len(data):
            if offset + 12 > len(data):
                break
            msg_len, feed_code, exchange_seg, sec_id, seq = struct.unpack_from(
                '<HBBII', data, offset
            )
            if msg_len == 0 or msg_len > len(data):
                self._parse_single_packet(data)
                break
            self._parse_single_packet(data[offset:offset + msg_len])
            offset += msg_len
            if offset >= len(data):
                break

    def _parse_single_packet(self, data):
        if len(data) < 12:
            return

        header = struct.unpack_from('<HBBII', data, 0)
        feed_code = header[1]

        payload = data[12:]
        levels = []
        for i in range(20):
            off = i * 16
            if off + 16 > len(payload):
                break
            price, qty, orders = struct.unpack_from('<dII', payload, off)
            levels.append({"price": round(price, 2), "qty": qty, "orders": orders})

        while len(levels) < 20:
            levels.append({"price": 0.0, "qty": 0, "orders": 0})

        now = time.time()

        if feed_code == FEED_CODE_BID:
            self._prev_bids = [b.copy() for b in self._bids]
            self._bids = levels
        elif feed_code == FEED_CODE_ASK:
            self._prev_asks = [a.copy() for a in self._asks]
            self._asks = levels
        else:
            return

        self._last_update = now
        self._total_bid_qty = sum(b["qty"] for b in self._bids)
        self._total_ask_qty = sum(a["qty"] for a in self._asks)

        # Analytics
        self._update_delta()
        self._reconstruct_ticks()
        self._track_wall_changes()

        # Throttled SocketIO emit
        if now - self._last_emit >= self._emit_interval:
            self._emit_update()
            self._last_emit = now

    # ── Analytics ────────────────────────────────────────────────

    def _calc_imbalance(self, bids, asks, depth):
        bid_qty = sum(b["qty"] for b in bids[:depth])
        ask_qty = sum(a["qty"] for a in asks[:depth])
        total = bid_qty + ask_qty

        if total == 0:
            return {"bid_qty": 0, "ask_qty": 0, "pct": 0.0, "text": "No Data"}

        pct = round(((bid_qty - ask_qty) / total) * 100, 1)

        if pct > 30:
            text = "Strong Buying"
        elif pct > 15:
            text = "Moderate Buying"
        elif pct > 5:
            text = "Slight Buying"
        elif pct < -30:
            text = "Strong Selling"
        elif pct < -15:
            text = "Moderate Selling"
        elif pct < -5:
            text = "Slight Selling"
        else:
            text = "Balanced"

        return {"bid_qty": bid_qty, "ask_qty": ask_qty, "pct": pct, "text": text}

    def _detect_walls(self, bids, asks):
        walls = {"bids": [], "asks": []}

        bid_qtys = [b["qty"] for b in bids if b["qty"] > 0]
        ask_qtys = [a["qty"] for a in asks if a["qty"] > 0]

        avg_bid = sum(bid_qtys) / len(bid_qtys) if bid_qtys else 0
        avg_ask = sum(ask_qtys) / len(ask_qtys) if ask_qtys else 0

        for i, b in enumerate(bids):
            if avg_bid > 0 and b["qty"] > avg_bid * WALL_THRESHOLD_MULTIPLIER:
                walls["bids"].append({
                    "level": i, "price": b["price"], "qty": b["qty"],
                    "orders": b["orders"],
                    "strength": round(b["qty"] / avg_bid, 1),
                })

        for i, a in enumerate(asks):
            if avg_ask > 0 and a["qty"] > avg_ask * WALL_THRESHOLD_MULTIPLIER:
                walls["asks"].append({
                    "level": i, "price": a["price"], "qty": a["qty"],
                    "orders": a["orders"],
                    "strength": round(a["qty"] / avg_ask, 1),
                })

        return walls

    def _update_delta(self):
        if not self._prev_bids or not self._prev_asks:
            return

        if (self._prev_bids[0]["price"] > 0
                and self._prev_bids[0]["price"] == self._bids[0]["price"]):
            change = self._prev_bids[0]["qty"] - self._bids[0]["qty"]
            if change > 0:
                self._cumulative_delta -= change

        if (self._prev_asks[0]["price"] > 0
                and self._prev_asks[0]["price"] == self._asks[0]["price"]):
            change = self._prev_asks[0]["qty"] - self._asks[0]["qty"]
            if change > 0:
                self._cumulative_delta += change

        self._delta_history.append({
            "t": round(time.time(), 1),
            "d": round(self._cumulative_delta),
        })

    def _reconstruct_ticks(self):
        if not self._prev_bids or not self._prev_asks:
            return

        now = round(time.time(), 1)

        if (self._prev_bids[0]["price"] > 0
                and self._prev_bids[0]["price"] == self._bids[0]["price"]):
            change = self._prev_bids[0]["qty"] - self._bids[0]["qty"]
            if change > 0:
                price = self._bids[0]["price"]
                self._tick_history.append({"t": now, "p": price, "v": change, "s": "sell"})
                vp = self._volume_profile.setdefault(price, {"buy": 0, "sell": 0})
                vp["sell"] += change

        if (self._prev_asks[0]["price"] > 0
                and self._prev_asks[0]["price"] == self._asks[0]["price"]):
            change = self._prev_asks[0]["qty"] - self._asks[0]["qty"]
            if change > 0:
                price = self._asks[0]["price"]
                self._tick_history.append({"t": now, "p": price, "v": change, "s": "buy"})
                vp = self._volume_profile.setdefault(price, {"buy": 0, "sell": 0})
                vp["buy"] += change

    def _get_volume_profile(self):
        if not self._volume_profile:
            return []
        return [
            {"price": p, "buy": v["buy"], "sell": v["sell"]}
            for p, v in sorted(self._volume_profile.items())
        ]

    def _track_wall_changes(self):
        if not self._prev_bids or not self._prev_asks:
            return

        now = time.time()
        self._check_wall_side(self._prev_bids[:5], self._bids[:5], "bid", now)
        self._check_wall_side(self._prev_asks[:5], self._asks[:5], "ask", now)

    def _check_wall_side(self, prev_levels, curr_levels, side, now):
        all_qty = [lev["qty"] for lev in prev_levels if lev["qty"] > 0]
        avg_qty = sum(all_qty) / len(all_qty) if all_qty else 0

        for prev, curr in zip(prev_levels, curr_levels):
            if prev["price"] <= 0 or prev["qty"] <= 0:
                continue
            if avg_qty <= 0 or prev["qty"] <= avg_qty * WALL_THRESHOLD_MULTIPLIER:
                continue

            if curr["price"] == prev["price"] and curr["qty"] > 0:
                eaten = prev["qty"] - curr["qty"]
                if eaten > prev["qty"] * 0.3:
                    self._alerts.appendleft({
                        "time": now, "type": "absorption", "side": side,
                        "price": prev["price"],
                        "prev_qty": prev["qty"], "curr_qty": curr["qty"],
                        "pct": round((eaten / prev["qty"]) * 100),
                    })
            elif curr["price"] != prev["price"] or curr["qty"] == 0:
                self._alerts.appendleft({
                    "time": now, "type": "pull", "side": side,
                    "price": prev["price"], "qty": prev["qty"],
                })

        # Stacking detection
        curr_all_qty = [lev["qty"] for lev in curr_levels if lev["qty"] > 0]
        curr_avg = sum(curr_all_qty) / len(curr_all_qty) if curr_all_qty else 0

        for curr in curr_levels:
            if curr["price"] <= 0 or curr["qty"] <= 0:
                continue
            if curr_avg <= 0 or curr["qty"] <= curr_avg * STACKING_THRESHOLD_MULTIPLIER:
                continue

            was_wall = False
            for prev in prev_levels:
                if prev["price"] == curr["price"]:
                    if avg_qty > 0 and prev["qty"] > avg_qty * STACKING_THRESHOLD_MULTIPLIER:
                        was_wall = True
                    break

            if not was_wall:
                self._alerts.appendleft({
                    "time": now, "type": "stacking", "side": side,
                    "price": curr["price"], "qty": curr["qty"],
                    "strength": round(curr["qty"] / curr_avg, 1) if curr_avg > 0 else 0,
                })

    # ── SocketIO Push ────────────────────────────────────────────

    def _emit_update(self):
        if not self._socketio:
            return
        try:
            self._socketio.emit("depth_update", self.get_snapshot())
        except Exception as e:
            logger.debug("Emit error: %s", e)
